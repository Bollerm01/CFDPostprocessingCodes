...
%% --- Ask user for optional x-axis ranges (Mach, Pressure, Velocity, TKE) ---
...
xRangeAns = inputdlg(...
    {...
     ...
     'TKE x-min (leave blank for auto):',     'TKE x-max (leave blank for auto):'...
    },...
...
if isempty(xRangeAns)
    ...
    % TKE
    xRangeTKE      = parseRange(xRangeAns{7}, xRangeAns{8}, 'TKE');
end
...
for f = 1:length(subfolders)
    ...
    excelFiles = orderfields(excelFiles);

    if numel(excelFiles) ~= nFiles
        error('Folder "%s" must contain exactly %d Excel files (found %d).',...
              subfolders(f).name, nFiles, numel(excelFiles));1
    end

    files = {excelFiles.name};
    % 1. Extract the trailing digits using regular expressions
    tokens = regexp(files, '(?<=_)[A-Za-z]*\d+', 'match', 'once');

    % 2. Strip any remaining letters to leave only the digits
    justDigits = regexp(tokens, '\d+', 'match', 'once');
    
    % 2. Convert extracted strings to numbers for accurate numeric sorting
    trailingNumbers = str2double(tokens);
    
    % 3. Sort the numbers and get the sorting index
    [~, sortIdx] = sort(trailingNumbers);
    
    % 4. Rearrange the original file names
    files = files(sortIdx);
    path  = folderPath;
    ...
    %% --- Interpolate onto a common Y grid of 250 pts---
    for i = 1:nFiles

        yFine = linspace(min(Y{i}),max(Y{i}),250).';
        ...
        TKE{i} = interp1(Y{i},TKE{i},yFine,'pchip');
    ...
    save_overlay(M, Y, legendLabels, savePrefix, saveSub,...
                 'Avg. Mach', sprintf('Avg. Mach, %s', savePrefix), 'mach', xRangeMach);

    % Pressure
    save_overlay(P, Y, legendLabels, savePrefix, saveSub,...
                 'Avg. Pressure (Pa)', sprintf('Avg. Pressure, %s', savePrefix), 'pressure', xRangePressure);

    % Velocity
    save_overlay(Vx, Y, legendLabels, savePrefix, saveSub,...
                 'Avg. Axial Velocity (m/s)', sprintf('Avg. Axial Velocity, %s', savePrefix), 'velocityx', xRangeVelocity);

    % TKE
    save_overlay(TKE, Y, legendLabels, savePrefix, saveSub,...
                 'TKE (m^2/s^2)', sprintf('TKE, %s', savePrefix), 'tke', xRangeTKE);
