...
%   - INDEPENDENT time filtering PER DATASET (A and B can use
%     different Start/End times, e.g. different simulation
%     start-up transients or run lengths)
...
%% ============================================================
% USER SETTINGS
%   Note: Start/End time is now specified INDEPENDENTLY for
%   Dataset A and Dataset B, since slice and full-domain CFD
%   runs may have different startup transients / run lengths.
%% ============================================================

answer = inputdlg( ...
    { ...
    'Plot Title', ...
    'Dataset A (Slice) Start Time [s]', ...
    'Dataset A (Slice) End Time [s]', ...
    'Dataset B (Full) Start Time [s]', ...
    'Dataset B (Full) End Time [s]', ...
    'FFT df [Hz]', ...
    'Min Frequency [Hz]', ...
    'Max Frequency [Hz]'}, ...
    'Settings', ...
...
tStart_A = str2double(answer{2});
tEnd_A   = str2double(answer{3});

tStart_B = str2double(answer{4});
tEnd_B   = str2double(answer{5});

df_desired = str2double(answer{6});
fmin = str2double(answer{7});
fmax = str2double(answer{8});
...
%% ============================================================
% PROCESS DATASET A  (uses tStart_A / tEnd_A)
%% ============================================================
...
%% ============================================================
% PROCESS DATASET B  (uses tStart_B / tEnd_B)
%% ============================================================
...
fprintf('Dataset A (Slice) time window: [%.3f, %.3f] s\n', tStart_A, tEnd_A);
fprintf('Dataset B (Full)  time window: [%.3f, %.3f] s\n', tStart_B, tEnd_B);
fprintf('%-35s  %10s\n', 'Name', 'OASPL [dB]');
fprintf('%s\n', repmat('-',1,50));
for i = 1:nA
