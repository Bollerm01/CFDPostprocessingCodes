...
%      in the top-left of the first tiled subplot.
%% ==========================================================
...
ax = gobjects(1,nAxial) ;         % to store axes
...
for a = 1:nAxial
    ...
    % Store handle to first subplot axes
    % if a == 2
    %     firstAx = gca;
    % end
    ax(a) = gca;
    ...
    if a == 1
        ...
        set(gca, 'YTickLabel', []);   % hides tick labels
...
%% ----------------- SHARED LEGEND (BUILT-IN TILE METHOD) ----
...
%% ----------------- COMPACT SPACE AND ADJUST POSITION -------
% 2. Free up white space inside the tiles so everything stays in the window frame
tl.Padding = 'tight'; 
tl.TileSpacing = 'tight';

% 3. Force the layout engine to respect manual positioning bounds
tl.PositionConstraint = 'outerposition';
tl.OuterPosition = [0.01, 0.01, 0.98, 0.98]; % [left, bottom, width, height]

%% ----------------- SHARED Y LABEL (INVISIBLE AXES) ----------
% 4. ALWAYS create the shared axes AFTER modifying tl.OuterPosition 
% so that tl.Position evaluates to the final, correct updated values.
drawnow; % Forces MATLAB to compute the new positions before running next line
