...
firstAx = [];          % to store handle to first subplot axes
...
%% ----------------- SHARED LEGEND (FIRST TILE, TOP-LEFT) ----
if ~isempty(allLineHandles) && ~isempty(firstAx) && isvalid(firstAx)
    lg = legend(firstAx, allLineHandles, legendEntries,...
                'Location', 'northwest',...  % top-left of first subplot
                'Box', 'on');                % box
...
% Vector PDF export
outputFile = 'AxialProfiles_VxNorm_Volcano.pdf';  % <-- vector PDF
