...
[filenames, pathname] = uigetfile({...
    ...
    'Select up to 4 Volcano probe Excel files (one per geometry)',...
    'MultiSelect', 'on');
...
if numel(filenames) > 4
    errordlg('You must select 4 or fewer Excel files. Please run again.','File Selection Error');
    return;
end
...
qtyNames  = {'TKE','AvgVorticityMag', 'AvgP', 'AvgV', 'Rxx','Ryy','Rzz'};
qtyFields = {'tke','vorticitymagavg', 'pressureavg', 'velocitymagavg', 'reynoldsstressxx','reynoldsstressyy','reynoldsstresszz'};
    ...
    for iA = 1:numel(axialLocs)
        ...
        for iQ = 1:numel(qtyNames)
            ...
            % These are now bold because of \mathbf in the string
             xlabel(xLabelStr, 'Interpreter','latex', 'FontSize', labelFontSize);
            ylabel(yLabelStr, 'Interpreter','latex', 'FontSize', labelFontSize);
            ...
            if ~isempty(legendEntries)
                lgd = legend(legendEntries,...
                             'Interpreter','none',...
                             'Location','best');
                set(lgd, 'FontWeight','normal','FontSize',legendFontSize);
            end
...
function geomData = readWorkbookForPlanes(excelFile, axialLocs)
    ...
    neededCols = {'Y_norm','reynoldsstressxx','reynoldsstressyy','reynoldsstresszz','tke', ...
        'vorticitymagavg', 'pressureavg', 'velocitymagavg'};
