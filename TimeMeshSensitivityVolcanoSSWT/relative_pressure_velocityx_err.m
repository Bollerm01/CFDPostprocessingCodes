function relative_pressure_velocityx_err()
    ...
    %% --- Threshold for small velocities ---
    epsilon = 10;  % velocities below this are considered "small" (~2% of flow)
    ...
    for f = 1:nFolders
        ...
        for i = 1:nFiles
            ...
            % --- X-Velocity (stable handling) ---
            relErrVx{i} = zeros(size(Vx{i}));
            smallIdx = abs(Vxref) <= epsilon;       % points with small reference velocity
            largeIdx = ~smallIdx;                    % points with sufficient reference velocity

            % relative % error for "large" velocities
            relErrVx{i}(largeIdx) = (Vx{i}(largeIdx) - Vxref(largeIdx)) ./ Vxref(largeIdx) * 100;
            % absolute difference for "small" velocities
            relErrVx{i}(smallIdx) = Vx{i}(smallIdx) - Vxref(smallIdx);
        ...
        %% --- Plot Absolute Relative % Error: X-Velocity with small-velocity points marked ---
        ...
        for i = 1:nFiles
            ...
            % Plot "large" reference points normally
            plot(abs(relErrVx{i}(~smallIdx)), Yref(~smallIdx), 'LineWidth', 2);

            % Plot "small" reference points as red circles
            plot(abs(relErrVx{i}(smallIdx)), Yref(smallIdx), 'ro', 'MarkerFaceColor','r');
        end
        xlabel('Absolute Relative X-Velocity Error (%) or Absolute Difference for Small Vx');
        ylabel('y (m)');
        xlim([0 10]);
        title(sprintf('Absolute Relative X-Velocity Error vs y (Ref: %s)', refFileName), 'Interpreter','none');
        legend(legendNames, 'Interpreter','none', 'Location','best');
        saveas(figVx, fullfile(saveSub, 'absolute_relative_velocityx_error_marked.fig'));
        print(figVx, fullfile(saveSub, 'absolute_relative_velocityx_error_marked.jpg'), '-djpeg','-r300');
        close(figVx);
